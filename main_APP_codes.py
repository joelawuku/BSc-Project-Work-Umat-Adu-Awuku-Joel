...
class DashboardTab(QWidget):
    ...
    def update_dashboard(self, max_rop, max_buckling, max_torque, max_drag, max_wob, max_rpm):
        ...
        self.max_buckling_value.setText(f"<font color='white' face='Courier New' size='5'>{max_buckling} lbf</font>")
        ...
        self.max_rpm_value.setText(f"<font color='white' face='Courier New' size='5'>{max_rpm} rev/min</font>")
...
class DrillstringCalculator:
    ...
    def calculate_parameters(self):
        ...
        rop_data = []
        rpm_data = [100] * len(self.inclination_values)  # Set RPM as a constant value (100 rev/min)
...
class MainWindow(QWidget):
        ...
        try:
            ...
            for idx, (x_val, y_val, z_val, inc_val, azi_val, md_val, dls_val) in enumerate(
                    zip(x_deviated, y_deviated, z_deviated, inclination[0], azimuth[0], measured_depth,
                        dogleg_angles_deg)):
                self.output_table.setItem(idx, 0, QTableWidgetItem(f"{x_val*3.281:.2f}"))
                self.output_table.setItem(idx, 1, QTableWidgetItem(f"{y_val*3.281:.2f}"))
                self.output_table.setItem(idx, 2, QTableWidgetItem(f"{z_val*3.281:.2f}"))
                ...
                self.output_table.setItem(idx, 5, QTableWidgetItem(f"{md_val*3.281:.2f}"))
    ...
    def display_drillstring_data(self, inclination_data, depth_data, buoyed_weight_data, weight_on_bit_data,
        ...
        for idx, (inclination, depth) in enumerate(zip(inclination_data, depth_data)):
            ...
            self.drillstring_table.setItem(idx, 1, QTableWidgetItem("{:.3f}".format(depth*3.281)))
